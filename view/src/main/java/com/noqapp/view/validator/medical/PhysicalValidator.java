...
@Component
public class PhysicalValidator implements Validator {
    ...
    @Autowired
    public PhysicalValidator(MedicalMasterDataService medicalMasterDataService) {
        this.medicalMasterDataService = medicalMasterDataService;
    }

    @Override
    public boolean supports(Class<?> clazz) {
        return PhysicalForm.class.equals(clazz);
    }
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (!errors.hasErrors()) {
            PhysicalForm form = (PhysicalForm) target;
            ...
            if (!errors.hasErrors()) {
                if (medicalMasterDataService.existsPhysical(form.getName().getText())) {
                    errors.rejectValue("name",
                            "name.exists",
