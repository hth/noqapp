...
@Component
public class RadiologyValidator implements Validator {
    private static final Logger LOG = LoggerFactory.getLogger(RadiologyValidator.class);

    private MedicalMasterDataService medicalMasterDataService;

    @Autowired
    public RadiologyValidator(MedicalMasterDataService medicalMasterDataService) {
        this.medicalMasterDataService = medicalMasterDataService;
    }

    @Override
    public boolean supports(Class<?> clazz) {
        return RadiologyForm.class.equals(clazz);
    }
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (!errors.hasErrors()) {
            RadiologyForm form = (RadiologyForm) target;
            ...
            if (!errors.hasErrors()) {
                if (medicalMasterDataService.existsRadiology(form.getName().getText())) {
                    errors.rejectValue("name",
                            "name.exists",
